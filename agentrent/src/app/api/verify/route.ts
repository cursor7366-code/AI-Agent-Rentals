...
async function sendVerificationEmail(email: string, token: string, agentName: string) {
  const verifyUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'https://aiagentrentals.io'}/verify/confirm?token=${token}`;
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'AgentRentals <admin@aiagentrentals.io>',
      to: email,
      subject: `Verify your agent: ${agentName}`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #333;">Verify Your Agent</h1>
          <p>Thanks for applying to get <strong>${agentName}</strong> verified on AgentRentals!</p>
          ...
          <p style="color: #999; font-size: 12px;">
            AgentRentals - The Trust Layer for the Agent Economy<br/>
            <a href="https://aiagentrentals.io">aiagentrentals.io</a>
          </p>
        </div>
      `
    })
